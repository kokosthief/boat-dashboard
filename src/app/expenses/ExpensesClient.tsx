...
export default function ExpensesClient({ data }: { data: ExpenseData }) {
  ...
  const categories = useMemo(() => ['All', ...Object.keys(data.categoryTotals).sort()], [data.categoryTotals]);
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [data.expenses, categoryFilter, paidByFilter, search]);
  ...
  // Chart data - exclude boat purchase for better visualization
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-blue-600 rounded-xl p-4">
          ...
          <p className="text-xl sm:text-2xl font-bold mt-1">{fmt(data.grandTotal)}</p>
          <p className="text-xs opacity-70 mt-1">{data.expenses.length} expenses</p>
        </div>
        <div className="bg-emerald-600 rounded-xl p-4">
          <p className="text-sm opacity-80">Excl. Boat Purchase</p>
          <p className="text-xl sm:text-2xl font-bold mt-1">{fmt(data.subtotalExPurchase)}</p>
        ...
        <div className="bg-pink-600 rounded-xl p-4">
          ...
          <p className="text-xl sm:text-2xl font-bold mt-1">{fmt(data.mumsContributions)}</p>
          <p className="text-xs opacity-70 mt-1">{data.expenses.filter(e => e.paidBy === 'Mum').length} expenses</p>
        </div>
        <div className="bg-amber-600 rounded-xl p-4">
          <p className="text-sm opacity-80">Henry&apos;s Net Cost</p>
          <p className="text-xl sm:text-2xl font-bold mt-1">{fmt(data.henrysNetCost)}</p>
      ...
      {/* Filtered total */}
      {(categoryFilter !== 'All' || paidByFilter !== 'All' || search) && (
